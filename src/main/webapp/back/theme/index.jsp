...
<script>
	$(function(){
		...
		$("#themeDataGrid").datagrid({
			url:'/kf/theme/queryAllByPage',//查询所有用户信息  json  rows total
			fit:true,
			rownumbers:true,
			pagination:true,
			fitColumns:true,
			...
			columns:[[
			         ...
			         {title:'主题名',field:'name',width:200,},
			         {title:'宣传图',field:'thumbnail',width:200,
                         formatter:function(value,row,index){
			             	if(value==null || value==""){
			             	    return "";
							}
                             return "<img src='"+value+"' style='width:100px;height:50px'>";
                         }
					 },
			         ...
			         {title:'操作',field:'options',width:200,
			        	formatter:function(value,row,index){
			        		return "<a href='javascript:;' class='btn' onClick=\"delThemeInfo('"+ row.id +"');\"  data-options=\"iconCls:'icon-remove'\" >删除</a>&nbsp;&nbsp;"
    ...
    function openSaveDialog(){
        ...
        $("#da").dialog({
            ...
            title:'添加主题信息',
            iconCls:'icon-man',
            href:'${pageContext.request.contextPath}/back/theme/save.jsp',//相当于两张页面的源码包含
            buttons:[{
                text:'保存信息',
                iconCls:'icon-save',
                handler:saveThemeInfo,
            },{
                text:'关闭',
                iconCls:'icon-cancel',
                handler:function(){
    ...
    function saveThemeInfo(){
        $("#saveThemeForm").form({
            ...
            onSubmit: function(){
                return $("#saveThemeForm").form('validate');
            },
            success:function(data){//data是一个json字符串 用时必须转为js对象
                ...
                //关闭对话框
                $("#da").dialog('close');
                $("#themeDataGrid").datagrid('reload',{});
            }
        });
        //提交form
        $("#saveThemeForm").form('submit');
    }

    //删除主题的信息
    function delThemeInfo(id){
        console.log(id);
        $.messager.confirm('提示', '确定要删除这条数据吗?', function(r){
            if (r){
                //发送ajax请求删除数据
                $.post("/kf/theme/delete",{"id":id},function(result){//原生ajaxjquery封装 获取的是js对象
                    //状态提示
                    //响应回来刷新刷新datagrid
                    $("#themeDataGrid").datagrid('reload');
                });
    ...
    function openUpdateDialog(id){
        ...
        $("#updateDialog").dialog({
            ...
            width:600,
            height:400,
            href:'${pageContext.request.contextPath}/back/theme/update.jsp?id='+id,
            buttons:[{
                text:'保存',
                iconCls:'icon-save',
                handler:updateThemeInfo,
            },{
                text:'关闭',
    ...
    function updateThemeInfo(){
        ...
        $("#updateThemeForm").form({
            url:'${pageContext.request.contextPath}/theme/update',
            onSubmit:function(){
    ...
    function delSelectRows(){
        $.messager.confirm('提示', '确定要删除这些数据吗?', function(r){
			if(r){
				//返回所有行数
				var rows = $("#themeDataGrid").datagrid('getChecked');
				if(rows.length>0){
...
<table id="themeDataGrid" class="easyui-datagrid"></table>
